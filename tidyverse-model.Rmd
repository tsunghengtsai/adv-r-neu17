...
```{r}
# One protein (AACT) measured in 3 runs (001_MZ_1, 002_MZ_1, 003_MZ_1)
sub_dia <- twin_dia2 %>% 
    filter(protein == "AACT", sample %in% c("001_MZ_1", "002_MZ_1", "003_MZ_1"))

# Linear model with fixed effects of sample and feature, no intercept
fit <- lm(log2inty_l_cn ~ 0 + sample + feature, data = sub_dia)
```
...
prots <- levels(twin_dia2$protein)
...
# Each row is an observation; new columns start with .
head(augment(fit))
