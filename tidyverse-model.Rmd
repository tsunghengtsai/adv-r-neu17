...
fit <- lm(log2inty_l_cn ~ 0 + run + feature, data = sub_dia)
```
...
for (i in seq_along(prots)) {
    ...
    fit <- lm(log2inty_l_cn ~ 0 + run + feature, data = oneprot)
    fit_coef <- coef(fit)
...
fit_dia <- twin_dia2 %>% 
    group_by(protein) %>% 
    do(fit = lm(log2inty_l_cn ~ 0 + run + feature, data = .))
fit_dia
```
...
twin_dia2 %>% 
    group_by(protein) %>%
    do(tidy(lm(log2inty_l_cn ~ 0 + run + feature, data = .))) %>% 
...
nested_dia <- nested_dia %>% 
    mutate(fit = map(data, ~ lm(log2inty_l_cn ~ 0 + run + feature, data = .)))
nested_dia
```
...
nested_dia <- nested_dia %>% 
    mutate(
        fit = map(data, ~ lm(log2inty_l_cn ~ 0 + run + feature, data = .)), 
        param = map(fit, tidy)
    )
