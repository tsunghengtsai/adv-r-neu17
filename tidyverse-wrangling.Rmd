...
library(stringr)
twin_dia2 %>% filter(str_detect(run, str_c(str_pad(1:10, 3, pad = "0"), collapse = "|"))) %>% 
...
The pronoun `.` is used as an argument placeholder, on which the function apply.  
...
If you use named arguments, each argument becomes a list-variable in the output:
...
A list-variable can contain any arbitrary R object which makes this form of do() useful for storing models:
...
This creates a list-column `fit_cor` to store the fitted models. We can use double bracket `[[]]` to retrieve the model objects:
