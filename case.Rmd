...
library(stringr)
sub <- tbl_df(twin_dia) %>% 
    filter(str_detect(sample, str_c(str_pad(1:10, 3, pad = "0"), collapse = "|"))) %>% 
    rename(heavy = intensity_h, light = intensity_l) %>% 
    gather(label, intensity, heavy, light)

sub %>% filter(protein == "APOA") %>% 
    ggplot(aes(sample, log2(intensity), group = feature, colour = feature)) + 
    geom_point() + 
    geom_line() + 
    ggtitle("APOA") + 
    facet_wrap(~ label) + 
...
sub %>% ggplot(aes(sample, log2(intensity))) + geom_boxplot() + 
    facet_wrap(~ label) + 
...
```{r}
# Filter based on columns intensity_h and sample
sub_dia <- twin_dia[!is.na(twin_dia$intensity_h) & twin_dia$sample == "001_MZ_1", ]
head(sub_dia)
```
...
Compute the median of feature intensities from heavy channel (`log2inty_h`) in one run (`001_MZ_1`)

```{r}
median(twin_dia$log2inty_h[!is.na(twin_dia$log2inty_h) & twin_dia$sample == "001_MZ_1"])
```

* The name of the data frame is repeated several times. This can be avoided by using `with()`, as in 
`with(twin_dia, median(log2inty_h[!is.na(log2inty_h) & sample == "001_MZ_1"]))`
...
To compute the medians of log-intensities in all runs, we need to split the column vector `log2inty_h` into subsets defined by `sample`, apply `median()` to each subset, and combine the results
...
```{r}
# Approach 1
runs <- unique(twin_dia$sample)

medians <- rep(0, length(runs))  # create a vector to restore the result
for (i in seq_along(runs)) {
    medians[i] <- median(twin_dia$log2inty_h[twin_dia$sample == runs[i]], na.rm = TRUE)
}
str(medians)
```
...
```{r}
# Approach 2
medians <- tapply(twin_dia$log2inty_h, twin_dia$sample, median, na.rm = TRUE)
...
```{r}
# Approach 3
df_median <- aggregate(twin_dia$log2inty_h, list(sample = twin_dia$sample), median, na.rm = TRUE)
...
for (ii in names(medians)) {
    log2inty_h <- twin_dia$log2inty_h[twin_dia$sample == ii]
    log2inty_l <- twin_dia$log2inty_l[twin_dia$sample == ii]
    twin_dia$log2inty_h[twin_dia$sample == ii] <- log2inty_h - medians[ii] + gbl_median
    twin_dia$log2inty_l[twin_dia$sample == ii] <- log2inty_l - medians[ii] + gbl_median
}
```
...
medians[c("001_MZ_1", "002_MZ_1", "001_MZ_1")]
```

```{r}
# Use vectorized representation to normalize the dataset
twin_dia$log2inty_h <- twin_dia$log2inty_h - medians[twin_dia$sample] + gbl_median
twin_dia$log2inty_l <- twin_dia$log2inty_l - medians[twin_dia$sample] + gbl_median
```
...
sum_t <- tapply(
    ...
    list(sample = twin_dia$sample, protein = twin_dia$protein), 
...
```{r}
sum_g$zyg <- ifelse(grepl("MZ", sum_g$sample), "MZ", "DZ")  # Create a new column for zygosity
sub_sum <- sum_g[sum_g$protein == "A2MG", ]  # Subset for protein A2MG

fit <- lm(log2inty ~ zyg, data = sub_sum)
```


```{r, eval=FALSE}
# Same as
fit <- lm(log2inty ~ zyg, data = sum_g, subset = sum_g$protein == "A2MG")
```
...
```{r}
ttest <- t.test(log2inty ~ zyg, data = sub_sum)
ttest
```
...
```{r, eval=FALSE}
t.test(log2inty ~ zyg, data = sum_g, subset = sum_g$protein == "A2MG")
```

Use two vectors for the two samples

```{r, eval=FALSE}
t.test(x = sub_sum$log2inty[sub_sum$zyg == "DZ"], y = sub_sum$log2inty[sub_sum$zyg == "MZ"])
```
